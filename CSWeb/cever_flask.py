...
#get all form data and set to local variable using request.args.get
    ...
    with open('sleep_data.csv', 'a', newline='') as csvfile: #open up the csv file sleep_data to write to it
        if csvfile.tell() == 0: #only write the title row if the file is empty
        ...
        #write out the data followed by a newline so the next entry starts on a fresh line
        csvfile.write(data + "\n")
